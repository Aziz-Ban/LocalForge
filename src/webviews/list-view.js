...
window.ListView = (() => {
  ...
  function formHtml(agentId, name, port, modelId, sysPrompt) {
    ...
    if (LF.models.length) {
      LF.models.forEach(m => {
        opts += '<option value="' + m.family + '"' + (m.family === modelId ? ' selected' : '') + '>' + LF.esc(m.name) + '</option>';
      });
    } else {
    ...
    h.push('    <button class="ib btn-in-cancel" title="Cancel"><svg viewBox="0 0 24 24"><path d="M19 6.41L17.59 5 12 10.59 6.41 5 5 6.41 10.59 12 5 17.59 6.41 19 12 13.41 17.59 19 19 17.59 13.41 12z"/></svg></button>');
    h.push('  </div>');
    h.push('  <div class="fc-body">');
    h.push('    <div class="field"><label>Agent Name</label>');
    h.push('      <input type="text" id="fn-' + agentId + '" value="' + LF.esc(name) + '" placeholder="e.g. Code Reviewer" autocomplete="off" /></div>');
    h.push('    <div class="field"><label>Model &amp; Port</label><div class="row">');
    h.push('      <select id="fm-' + agentId + '">' + opts + '</select>');
    h.push('      <input type="number" id="fp-' + agentId + '" value="' + port + '" min="1024" max="65535" /></div></div>');
    h.push('    <div class="field"><label class="tgl">');
    h.push('      <input type="checkbox" id="fchk-' + agentId + '" ' + (checked ? 'checked' : '') + ' />');
    h.push('      <span class="tgl-text">System Prompt</span></label>');
    h.push('      <div class="slide ' + (checked ? 'open' : '') + '" id="fsl-' + agentId + '">');
    h.push('        <textarea id="fx-' + agentId + '" class="ctx" placeholder="e.g. You are a senior engineer.">' + LF.esc(sysPrompt) + '</textarea>');
    h.push('      </div></div>');
    h.push('    <div class="btn-row">');
    h.push('      <button class="btn btn-ghost btn-in-cancel">Cancel</button>');
    h.push('      <button class="btn btn-in-save" data-sid="' + agentId + '">' + (agentId === 'new' ? 'Create Agent' : 'Save Changes') + '</button>');
    h.push('    </div>');
    h.push('  </div>');
    h.push('</div>');
    return h.join('\n');
    ...
    visible.forEach((a) => {
      ...
      if (LF.editingId === a.id) {
        ...
        card.innerHTML =
          ...
          '<div class="ag-sub">Port ' + a.port + ' · ' + LF.esc(a.modelId || 'default') + '</div>' +
          '</div>' +
          '<div class="ag-actions">' +
          (isOn
            ? '<button class="ab st" data-stop="' + a.id + '">Stop</button><button class="ab" data-info="' + a.id + '">ℹ</button>'
            : '<button class="ab go" data-start="' + a.id + '">Start</button><button class="ab" data-edit="' + a.id + '">✎</button><button class="ab del" data-del="' + a.id + '">✕</button>'
          ) +
          '</div></div>' +
          '<div class="ag-ep">' +
  ...
  document.getElementById('agent-list').addEventListener('click', (e) => {
    ...
    if (t.dataset.newAgent) {
      LF.editingId = 'new';
      render();
      setTimeout(() => { const el = document.getElementById('fn-new'); if (el) el.focus(); }, 50);
    }
    ...
    if (t.dataset.del) {
      ...
      LF.showConfirm(
        ...
        () => { LF.agents = LF.agents.filter(x => x.id !== t.dataset.del); LF.persistAgents(); render(); LF.toast('Agent removed', 'success'); }
      );
    }

    if (t.dataset.copy) {
      navigator.clipboard.writeText(t.dataset.copy).then(() => {
        t.textContent = 'Done'; t.classList.add('done');
        setTimeout(() => { t.textContent = 'Copy'; t.classList.remove('done'); }, 1500);
      });
    }

    if (t.classList.contains('btn-in-cancel')) {
      LF.editingId = null; render();
    }
    ...
    if (t.classList.contains('btn-in-save')) {
      ...
      if (!name) { LF.toast('Agent name is required', 'error'); nameEl.focus(); return; }
      const port = parseInt(portEl.value);
      if (!port || port < 1024 || port > 65535) { LF.toast('Port must be 1024-65535', 'error'); portEl.focus(); return; }
      ...
      if (sid === 'new') {
        ...
        LF.agents.push({ id: LF.uid(), projectId: projId, name, port, modelId, systemPrompt, running: false });
